...
class Solution
{
  ...
  function calculateTax($brackets, $income)
  {
    ...
    $tax_paid = 0;
    foreach ($brackets as $step => $bracket) {

      $income_limit_for_current_bracket = $bracket[0];
      $tax_rate = $bracket[1];

      // if current bracket income limit is greater than income
      // to set income in bracket to $income less income limit of prior value
      if ($income_limit_for_current_bracket > $income) {
        if ($step === 0) {
          $income_in_tax_bracket = $income;
        } else {
          $income_in_tax_bracket = $income - $brackets[$step - 1][0];
        }
      } else {
        if ($step === 0) {
          $income_in_tax_bracket = $income_limit_for_current_bracket;
        } else {
          $income_in_tax_bracket = $income_limit_for_current_bracket - $brackets[$step - 1][0];
        }
      }
      ...
      $tax_for_current_bracket = $income_in_tax_bracket * ($tax_rate / 100);

      $tax_paid += $tax_for_current_bracket;
      if ($income_limit_for_current_bracket >= $income) {
        break;
      }
    }
...
(new Solution())->calculateTax($brackets, $income);
